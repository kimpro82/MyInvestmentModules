...
Private Sub XAQuery_t1101_ReceiveData(ByVal szTrCode As String)
    ...
    Sheet2.Range("R3") = XAQuery_t1101.GetFieldData("t1101OutBlock", "hname", 0)                            ' 종목명
    Sheet2.Range("Q5") = XAQuery_t1101.GetFieldData("t1101OutBlock", "price", 0)                            ' 현재가
    Dim sSign As String
    sSign = GetSign(XAQuery_t1101.GetFieldData("t1101OutBlock", "sign", 0))                                 ' 전일대비구분 (※ 별도 함수 GetSign() 정의 필요)
    Sheet2.Range("S5") = sSign & XAQuery_t1101.GetFieldData("t1101OutBlock", "change", 0)                   ' 전일대비
    Sheet2.Range("T5") = XAQuery_t1101.GetFieldData("t1101OutBlock", "diff", 0) / 100                       ' 등락률
    Sheet2.Range("U5") = XAQuery_t1101.GetFieldData("t1101OutBlock", "volume", 0)                           ' (당일)누적거래량

    Dim i As Integer
    For i = 1 To 10
        
        Sheet2.Range("S" & (6 + 10 - i)) = XAQuery_t1101.GetFieldData("t1101OutBlock", "offerho" & i, 0)    ' 매도호가
        Sheet2.Range("S" & (6 + 9 + i)) = XAQuery_t1101.GetFieldData("t1101OutBlock", "bidho" & i, 0)       ' 매수호가
